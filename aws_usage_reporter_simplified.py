#!/usr/bin/env python3
"""
AWS Usage Report Generator - Simplified Version
Returns only essential metrics as requested:
- In-Use Capacity and Instance Count for each service
- For S3: also includes Object Count
- Improved S3 processing for large bucket counts

Version: 4.0 - Simplified Essentials
...
class AWSUsageReporter:
    """Simplified AWS Usage Reporter - Essential Metrics Only"""
    ...
    def _get_bucket_metrics_parallel(self, bucket_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            start_time = end_time - timedelta(days=7)  # Extended range

            size_gb = 0
            object_count = 0

            # Try to get size metrics
    ...
    def get_s3_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            max_workers = min(20, bucket_count)  # Limit concurrent requests
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in as_completed(future_to_bucket, timeout=300):  # 5 minute timeout
                    try:
                        result = future.result(timeout=30)  # 30 second per bucket timeout
                        if result['has_data']:
    ...
    def get_storage_gateway_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'In_Use_Capacity_GB': 0,  # Would need additional API calls for actual storage
                'Region': self.region
            }
        except Exception as e:
    ...
    def get_documentdb_metrics(self) -> Dict[str, Any]:
        """DocumentDB: Instance Count + In-Use Capacity"""
        try:
            self.logger.info("📊 Collecting DocumentDB essentials...")

            instances = self._paginate_all(self.docdb_client, 'describe_db_instances', 'DBInstances')
            instance_count = len(instances)

            return {
                'Service': 'DocumentDB',
                'Instance_Count': instance_count,
                'In_Use_Capacity_GB': 0,  # Storage info would need additional processing
                'Region': self.region
            }
        except Exception as e:
    ...
    def get_rds_metrics(self) -> Dict[str, Any]:
        """RDS: Instance Count + In-Use Capacity (Allocated Storage)"""
        try:
            self.logger.info("📊 Collecting RDS essentials...")

            instances = self._paginate_all(self.rds_client, 'describe_db_instances', 'DBInstances')
            instance_count = len(instances)
            total_storage = sum(inst.get('AllocatedStorage', 0) for inst in instances)
    ...
    def get_neptune_metrics(self) -> Dict[str, Any]:
        """Neptune: Instance Count + In-Use Capacity"""
        try:
            self.logger.info("📊 Collecting Neptune essentials...")

            instances = self._paginate_all(self.neptune_client, 'describe_db_instances', 'DBInstances')
            instance_count = len(instances)
    ...
    def get_dynamodb_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Sample tables for size estimation
            total_size = 0
            ...
            if sample_size > 0:
                ...
                estimated_total = (avg_size * table_count) / (1024**3)  # Convert to GB
            else:
                estimated_total = 0
    ...
    def generate_report(self) -> List[Dict[str, Any]]:
        """Generate simplified report with essential metrics only"""
        self.logger.info("🚀 Starting AWS Usage Report - Essential Metrics Only")
        self.logger.info("=" * 70)
        ...
        service_methods = [
            ...
            self.get_s3_metrics,  # Improved S3 processing
    ...
    def save_to_csv(self, filename: str = None) -> str:
        """Save simplified report to CSV"""
        if not filename:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"aws_usage_essentials_{self.region}_{timestamp}.csv"

        try:
            with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
                # Define essential fieldnames only
            ...
            self.logger.info(f"✅ Simplified report saved: {filename}")
            return filename
    ...
    def print_summary(self):
        ...
        print("\n" + "=" * 80)
        print("📋 AWS USAGE REPORT - ESSENTIAL METRICS ONLY")
        print("=" * 80)
        print(f"Region: {self.region}")
        print(f"Services: {len(self.report_data)}")
        print("\n" + "-" * 80)
        ...
        for record in self.report_data:
            ...
            # Get capacity with appropriate unit
...
def main():
    ...
    parser = argparse.ArgumentParser(description='AWS Usage Report - Essential Metrics Only')
    ...
    try:
        logger.info("🚀 AWS Usage Reporter - Essential Metrics Only")
        logger.info("📊 Collecting: Instance Count + In-Use Capacity (+ S3 Object Count)")
        ...
        logger.info(f"\n✅ Essential metrics report complete: {csv_file}")
        return 0
