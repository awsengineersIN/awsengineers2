                ...
                try:
                    ...
                    if count > 0:
                        groups.append({
                            ...
                            'Non-Compliant': resp.get('InstancesWithFailedPatches', 0),
                            'Unspecified': resp.get('InstancesWithUnreportedNotApplicablePatches', 0)
                        })
                except:
                    pass
    except Exception as e:
...
if not st.session_state.pc_data:
    ...
    if inst_df.empty and grp_df.empty and pat_df.empty:
        ...
        with m2:
            st.metric("🔴 Non-Compliant", miss + fail)
        with m3:
        ...
        with tab2:
            ...
            if not filtered_inst.empty:
                display_cols = ['Instance ID', 'Instance Name', 'Platform', 'Compliance Status', 'Managed', 'Installed Patches', 'Missing Patches', 'Failed Patches', 'Instance State', 'Launch Time Display', 'Account Name', 'Region']
