...
def fetch_account_region_data(account_id, account_name, region, role):
        ...
        for page in paginator.paginate():
            for res in page.get('Reservations', []):
                for inst in res.get('Instances', []):
                    iid = inst['InstanceId']
                    platform = inst.get('Platform', 'linux')  # Default to linux if not specified
                    instance_map[iid] = {
    ...
    # Get patch states - FIXED: Batch API calls to avoid InstanceIds parameter validation
    try:
        if managed_ids:
            ...
            for i in range(0, len(managed_list), batch_size):
                ...
                try:
                    ...
                    errors.append(f"⚠️ {account_name}/{region}: Batch patch states - {str(batch_err)[:50]}")
    except Exception as e:
    ...
    # Get patch groups (Compliance Summary) - FIXED: Proper filtering to avoid empty rows
    try:
        ...
        for page in paginator.paginate():
            for group in page.get('Mappings', []):
                ...
                try:
                    ...
                    # Only include if count > 0 and has actual data
                    if count > 0 and (compliant > 0 or non_compliant > 0 or unspecified > 0):
                        groups.append({
                            'Account Name': account_name,
                            'Region': region,
...
if not st.session_state.pc_data:
    ...
    if inst_df.empty and grp_df.empty and pat_df.empty:
        ...
        unspec = len(inst_df[inst_df['Compliance Status'] == 'UNSPECIFIED']) if not inst_df.empty else 0
        ...
        total_failed = int(inst_df['Failed Patches'].sum()) if not inst_df.empty else 0
        ...
        # Compliance Summary
        with c2:
            comp_data = [comp, non_comp, unspec, unmg]
            comp_labs = ['Compliant', 'Non-Compliant', 'Unspecified', 'Unmanaged']
            comp_cols = ['#28a745', '#dc3545', '#fd7e14', '#6c757d']
        ...
        with c3:
            if not filtered.empty:
                ...
                fail_cnt = len(filtered[filtered['Failed Patches'] > 0])
        ...
        with tab1:
            ...
            if not grp_df.empty:
                ...
                st.info("ℹ️ No compliance summary data available.")
        
        with tab2:
            st.subheader("Instance Patch Compliance Report")
            
            if not filtered.empty:
                display_cols = ['Instance ID', 'Instance Name', 'Platform', 'Compliance Status', 'Managed', 'Installed Patches', 'Missing Patches', 'Failed Patches', 'Instance State', 'Account Name', 'Region']
        ...
        with tab4:
            ...
            if not inst_df.empty:
                ...
                if not missing_patches_df.empty:
                    display_cols = ['Instance ID', 'Instance Name', 'Account Name', 'Region', 'Missing Patches', 'Severity']
                    if 'Severity' not in missing_patches_df.columns:
                        missing_patches_df['Severity'] = 'N/A'
