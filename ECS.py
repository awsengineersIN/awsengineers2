            ...
            for cluster_arn in clusters_resp.get('clusterArns', []):
                ...
                for cluster in cluster_detail['clusters']:
                    resources['clusters'].append({
                        ...
                        'Running Count': cluster.get('runningCount', 0),
                        'Pending Count': cluster.get('pendingCount', 0),
                ...
                if tasks_resp.get('taskArns'):
                    ...
                    for task in task_detail['tasks']:
                        resources['tasks'].append({
                            ...
                            'Created At': task.get('createdAt', 'N/A'),
...
if st.session_state.ecs_data['clusters'] is not None:
    ...
    with tab1:
        ...
        if df_clusters.empty:
            ...
            with col2:
                st.metric("Running Tasks", df_clusters['Running Count'].sum())
            with col3:
                st.metric("Pending Tasks", df_clusters['Pending Count'].sum())
    ...
    with tab2:
        ...
        if df_services.empty:
            ...
            with col2:
                st.metric("Running Tasks", df_services['Running Count'].sum())
            with col3:
                st.metric("Pending Tasks", df_services['Pending Count'].sum())
