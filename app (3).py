...
if hierarchy is None:
    ...
    with st.expander("🌳 View Organization Hierarchy", expanded=False):
        ...
        def build_hierarchy_text(node, prefix="", is_last=True):
            """Build hierarchy as text list"""
            
            if node['Type'] == 'ROOT':
                hierarchy_text.append(f"🏢 **{node['Name']}** (Root)")
                ...
                for idx, (item_type, item) in enumerate(all_items):
                    ...
                    if item_type == 'OU':
                        render_ou_text(item, "  ", is_last_item)
                    else:
                        render_account_text(item, "  ", is_last_item)
        
        def render_ou_text(ou, prefix="", is_last=True):
            """Render OU as text"""
            connector = "└── " if is_last else "├── "
            next_prefix = prefix + ("    " if is_last else "│   ")
            
            hierarchy_text.append(f"{prefix}{connector}📁 **{ou['Name']}**")
            ...
            for idx, (item_type, item) in enumerate(all_items):
                is_last_item = idx == len(all_items) - 1
                if item_type == 'OU':
                    render_ou_text(item, next_prefix, is_last_item)
                else:
                    render_account_text(item, next_prefix, is_last_item)
        
        def render_account_text(account, prefix="", is_last=True):
            """Render account as text"""
            connector = "└── " if is_last else "├── "
            hierarchy_text.append(f"{prefix}{connector}👤 **{account['Name']}** ({account['Email']})")
        
        build_hierarchy_text(hierarchy)
        
        # Display the hierarchy
        for line in hierarchy_text:
            st.write(line)
