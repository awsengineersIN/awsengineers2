...
if st.session_state.alarms_data is not None:
    ...
    if df.empty:
        ...
        with tab1:
            ...
            display_df = filtered[display_cols].sort_values('State').reset_index(drop=True)
        ...
        with tab2:
            ...
            if triggered.empty:
                ...
                display_df = triggered[['Alarm Name', 'In Alarm Since', 'Description', 'Namespace', 'Metric Name', 'Threshold', 'Account Name', 'Region']].sort_values('Alarm Name').reset_index(drop=True)
        ...
        with tab3:
            ...
            try:
                ...
                recent = df_copy[df_copy['StateUpdatedTimestamp'] >= cutoff].sort_values('StateUpdatedTimestamp', ascending=False).reset_index(drop=True)
            except:
                recent = df_copy.sort_values('State Updated', ascending=False).head(50).reset_index(drop=True)
