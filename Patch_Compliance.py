...
def get_patch_compliance(account_id, account_name, region, role_name):
    ...
    try:
                ...
                for patch_group in page.get('Mappings', []):
                    ...
                    try:
                        ...
                        summary_data.append({
                            ...
                            'Instances Count': compliance_resp.get('Instances', 0),
                            'Compliant': compliance_resp.get('InstancesWithInstalledPatches', 0),
                            'Non-Compliant': compliance_resp.get('InstancesWithFailedPatches', 0),
                            'Unspecified': compliance_resp.get('InstancesWithUnreportedNotApplicablePatches', 0),
                            'Last Update': compliance_resp.get('InstancesWithNotApplicablePatches', 0)
                        })
        ...
        # ==================== INSTANCE COMPLIANCE ====================
        try:
            # Get all instances with patch manager
            ...
            # Get compliance status for each instance
            if instance_ids:
                compliance_paginator = ssm.get_paginator('list_compliance_items')
                compliance_pages = compliance_paginator.paginate(
                    Filters=[
                        {'Key': 'Status', 'Values': ['COMPLIANT', 'NON_COMPLIANT', 'UNSPECIFIED']}
                    ]
                )
                ...
                for page in compliance_pages:
                    for item in page.get('ComplianceItems', []):
                        ...
                        if resource_id in instance_map:
                            compliance_type = item.get('ComplianceType', 'Patch')
                            ...
                            instance_data.append({
                                ...
                                'Instance Name': instance_map[resource_id]['Name'],
                                'Platform': instance_map[resource_id]['Platform'],
                                'Compliance Status': status,
                                'Launch Time': instance_map[resource_id]['LaunchTime'],
                                'Instance State': instance_map[resource_id]['State'],
                                'Severity': item.get('Details', {}).get('Severity', 'N/A')
                            })
            
        except Exception as e:
            errors.append(f"⚠️ {account_name}/{region}: Error fetching instance compliance - {str(e)}")
            ...
            for page in page_iterator:
                for patch in page.get('Patches', []):
                    patch_data.append({
                        ...
                        'Release Date': patch.get('ReleaseDate', 'N/A'),
...
if st.session_state.patch_data['summary'] is not None:
    ...
    if summary_df.empty and instance_df.empty and patch_df.empty:
        ...
        unspecified_count = len(instance_df[instance_df['Compliance Status'] == 'UNSPECIFIED']) if not instance_df.empty else 0
        ...
        with col3:
            st.metric("🟡 Unspecified", unspecified_count)
        with col4:
        ...
        if not filtered.empty:
            ...
            with graph_col1:
                ...
                colors = {'COMPLIANT': '#28a745', 'NON_COMPLIANT': '#dc3545', 'UNSPECIFIED': '#ffc107'}
                fig_status = px.pie(
        ...
        with tab1:
            ...
            if not summary_df.empty:
                ...
                st.info("No patch group data available.")
        ...
        with tab2:
            ...
            if not filtered.empty:
                display_cols = ['Instance ID', 'Instance Name', 'Platform', 'Compliance Status', 'Instance State', 'Severity', 'Launch Time', 'Account Name', 'Region']
        ...
        with tab4:
            ...
            if not patch_df.empty:
                ...
                st.info("No patch severity data available.")
